  ...
  return (
    <div className="space-y-4">
        ...
        <div className="space-y-4">
          <div className="bg-white p-4 rounded-2xl border border-stone-200 shadow-xs">
            ...
            <div className="flex items-baseline gap-2">
              ...
              <span className="text-xs text-stone-500">/ 21 pontos</span>
            </div>
          ...
          <div className="bg-white rounded-2xl border border-stone-200 shadow-xs p-4 sm:p-5">
            <div className="flex items-center justify-between mb-4">
              <h4 className="font-bold text-sm text-stone-800">Histórico de Avaliações PSQI</h4>
              <button
            ...
            {psqiList.length === 0 ? (
              ...
              <div className="divide-y divide-stone-100">
                {[...psqiList].reverse().map((p) => (
                  <div key={p.id} className="py-3 flex items-center justify-between text-xs hover:bg-stone-50 px-2 rounded-lg">
                    <div>
                      <p className="font-semibold text-stone-900">{fmtMesAno(p.mes)}</p>
                      <p className="text-stone-500 text-[11px]">
                        Score: {p.global ?? '–'} · {p.classificacao}
                      </p>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className="font-serif font-bold text-base text-stone-800">{p.global}</span>
                      <button
        ...
        <div className="space-y-4">
          <div className="bg-white p-4 rounded-2xl border border-stone-200 shadow-xs">
            ...
            <div className="flex items-baseline gap-2">
              ...
              <span className="text-xs text-stone-500">/ 24 pontos</span>
            </div>
            <p className="text-xs font-semibold text-emerald-800 mt-2">
          ...
          <div className="bg-white rounded-2xl border border-stone-200 shadow-xs p-4 sm:p-5">
            <div className="flex items-center justify-between mb-4">
              <h4 className="font-bold text-sm text-stone-800">Histórico de Avaliações Epworth</h4>
              <button
            ...
            {essList.length === 0 ? (
              ...
              <div className="divide-y divide-stone-100">
                {[...essList].reverse().map((e) => (
                  <div key={e.id} className="py-3 flex items-center justify-between text-xs hover:bg-stone-50 px-2 rounded-lg">
                    <div>
                      <p className="font-semibold text-stone-900">{fmtMesAno(e.mes)}</p>
                      <p className="text-stone-500 text-[11px]">{e.classificacao}</p>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className="font-serif font-bold text-base text-stone-800">{e.total}</span>
                      <button
      ...
      {showPsqiModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-xs">
          <div className="bg-white w-full max-w-lg rounded-2xl p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
            <h3 className="font-serif font-bold text-lg text-stone-900 mb-2">Questionário PSQI</h3>
            <p className="text-xs text-stone-500 mb-4">Referente aos hábitos do último mês</p>

            <form onSubmit={handleSalvarPSQI} className="space-y-3 text-xs">
              <div>
                <label className="font-semibold text-stone-700">Mês de Referência</label>
                <input
                  type="month"
                  value={mesPsqi}
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="font-semibold text-stone-700">Hora de Deitar (P1)</label>
                  <input
                    type="time"
                    value={horaDeitar}
                ...
                <div>
                  <label className="font-semibold text-stone-700">Hora de Acordar (P3)</label>
                  <input
                    type="time"
                    value={horaAcordar}
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="font-semibold text-stone-700">Latência (min) (P2)</label>
                  <input
                    type="number"
                    value={latenciaMin}
                ...
                <div>
                  <label className="font-semibold text-stone-700">Horas Dormidas (P4)</label>
                  <input
                    type="number"
                    step="0.5"
                    value={horasDormidas}
              ...
              <div>
                <label className="font-semibold text-stone-700">Qualidade Subjetiva do Sono (P6)</label>
                <select
                  value={p6}
              ...
              <div className="flex gap-2 pt-3">
                <button
                  ...
                  className="flex-1 py-2.5 rounded-xl bg-stone-100 font-semibold text-stone-700"
                >
                  Cancelar
                </button>
                <button
                  type="submit"
                  className="flex-1 py-2.5 rounded-xl bg-emerald-800 text-white font-bold"
                >
                  Calcular e Salvar
                </button>
              </div>
      ...
      {showEssModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-xs">
          <div className="bg-white w-full max-w-lg rounded-2xl p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
            <h3 className="font-serif font-bold text-lg text-stone-900 mb-1">Escala de Epworth</h3>
