    ...
    <div className="space-y-4">
      ...
      {subTab === 'acwr' && (
        <div className="space-y-4">
          ...
          <div className="p-3.5 rounded-xl bg-emerald-50/80 border border-emerald-200/80 text-xs text-emerald-950 flex items-start gap-2.5 leading-relaxed">
          ...
          <div className="bg-white p-4 sm:p-5 rounded-2xl border border-stone-200 shadow-xs">
            ...
            <div className="flex items-center gap-4 text-xs text-stone-500 mb-4">
              <span className="flex items-center gap-1.5">
                <span className="w-2.5 h-2.5 rounded-full bg-emerald-600 inline-block" /> Aguda (7d)
              </span>
              <span className="flex items-center gap-1.5">
                <span className="w-2.5 h-2.5 rounded-full bg-stone-400 inline-block" /> Crônica (28d)
              </span>
            </div>
            ...
            {cargaDiaria.length < 2 ? (
              ...
              <div className="h-48 w-full flex items-end gap-1 pt-4 pb-2 border-b border-stone-200">
                    ...
                    <div key={d.data} className="flex-1 flex flex-col items-center gap-0.5 group relative">
                      <div className="w-full flex items-end justify-center gap-0.5 h-36">
                        <div
                          ...
                          className="w-1/2 bg-emerald-600 rounded-t-xs transition-all group-hover:bg-emerald-700"
                        />
                        <div
                          style={{ height: `${Math.max(4, hCronica)}%` }}
                          className="w-1/2 bg-stone-300 rounded-t-xs transition-all group-hover:bg-stone-400"
                        />
                      </div>
                      <span className="text-[9px] text-stone-400 truncate w-full text-center mt-1">
                        {fmtBR(d.data).slice(0, 5)}
                      </span>
                      {/* Tooltip */}
                      <div className="absolute bottom-full mb-1 hidden group-hover:block z-10 bg-stone-900 text-white text-[10px] p-2 rounded-md shadow whitespace-nowrap pointer-events-none">
          ...
          <div className="bg-white rounded-2xl border border-stone-200 shadow-xs overflow-hidden">
            <div className="p-4 border-b border-stone-200">
              <h4 className="font-bold text-sm text-stone-800">Histórico de Carga Diária</h4>
            </div>
              ...
              <table className="w-full text-left text-xs">
                ...
                <tbody className="divide-y divide-stone-100 text-stone-700">
                  {[...cargaDiaria].reverse().map((d) => (
                    <tr key={d.data} className="hover:bg-stone-50">
        ...
        <div className="space-y-4">
          <div className="bg-white p-4 rounded-2xl border border-stone-200 shadow-xs">
            ...
            <select
              ...
              className="w-full px-3.5 py-2 rounded-lg border border-stone-300 text-stone-800 text-sm font-semibold"
            >
              {semanas.map((s) => (
                  ...
                  return (
                    <div key={g.grupo} className="space-y-1">
                      <div className="flex items-center justify-between text-xs">
                        <span className="font-bold text-stone-800">{g.grupo}</span>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white p-4 sm:p-5 rounded-2xl border border-stone-200 shadow-xs">
            ...
            {progresso.length < 2 ? (
              ...
              <div className="h-44 flex items-end gap-1.5 pt-4 pb-2 border-b border-stone-200">
                {progresso.map((p) => {
                  ...
                  return (
                    <div key={p.data} className="flex-1 flex flex-col items-center gap-1 group relative">
                      <div
                        ...
                        className="w-full bg-emerald-700 hover:bg-emerald-800 rounded-t-xs transition-all"
                      />
                      <span className="text-[9px] text-stone-500 truncate w-full text-center">
                        {fmtBR(p.data).slice(0, 5)}
                      </span>
                      <div className="absolute bottom-full mb-1 hidden group-hover:block z-10 bg-stone-900 text-white text-[10px] p-1.5 rounded-md shadow whitespace-nowrap pointer-events-none">
          ...
          <div className="bg-white rounded-2xl border border-stone-200 shadow-xs overflow-hidden">
            <div className="p-4 border-b border-stone-200">
              <h4 className="font-bold text-sm text-stone-800">Melhores Cargas Registradas (PRs)</h4>
            </div>
            <div className="divide-y divide-stone-100">
              {recordesPessoais.map((r) => (
                <div key={r.exercicio} className="p-3.5 flex items-center justify-between text-xs hover:bg-stone-50">
                  <div>
                    <p className="font-bold text-stone-900">{r.exercicio}</p>
                    <p className="text-stone-500 text-[11px]">{r.grupo} · Batido em {fmtBR(r.data)}</p>
                  </div>
                  <div className="text-right">
                    <p className="font-serif font-bold text-base text-emerald-800">{r.carga} kg</p>
                    {r.reps && <p className="text-[11px] text-stone-500">{r.reps} reps</p>}
                  </div>
                </div>
              ))}
            </div>
      ...
      {subTab === 'stats' && (
        <div className="space-y-4">
          <div className="bg-white rounded-2xl border border-stone-200 shadow-xs overflow-hidden p-4 sm:p-5">
            <h4 className="font-bold text-sm text-stone-800 mb-3">Estatísticas Descritivas</h4>
              ...
              <table className="w-full text-left text-xs">
                ...
                <tbody className="divide-y divide-stone-100 text-stone-700">
                  {[
                    ...
                    .map(([nome, stat]: any) => (
                      <tr key={nome}>
                        <td className="p-2.5 font-medium text-stone-900">{nome}</td>
                        <td className="p-2.5">{stat.n}</td>
                        <td className="p-2.5 font-semibold">{stat.media.toFixed(1)}</td>
                        <td className="p-2.5 text-stone-500">±{stat.dp.toFixed(1)}</td>
